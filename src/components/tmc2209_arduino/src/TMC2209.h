...
#include <Arduino.h>
    ...
    !defined(ARDUINO_ARCH_RP2040) && !defined(ARDUINO_SAM_DUE)
...
class TMC2209 {
 public:
  ...
  void setup(HardwareSerial& serial, long serial_baud_rate = 115200,
             SerialAddress serial_address = SERIAL_ADDRESS_0);
  // Alternate rx and tx pins may be specified for certain microcontrollers e.g.
  // ESP32 and RP2040
#if defined(ESP32)
  void setup(HardwareSerial& serial, long serial_baud_rate,
             SerialAddress serial_address, int16_t alternate_rx_pin,
             int16_t alternate_tx_pin);
#elif defined(ARDUINO_ARCH_RP2040)
  void setup(SerialUART& serial, long serial_baud_rate,
             SerialAddress serial_address, int16_t alternate_rx_pin,
             int16_t alternate_tx_pin);
#endif
  ...
  void setup(SoftwareSerial& serial, long serial_baud_rate = 9600,
             SerialAddress serial_address = SERIAL_ADDRESS_0);
#endif
