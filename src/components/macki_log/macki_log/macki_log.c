...
log_manager_status_t macki_log(log_level_t level, const char* tag,
  ...
  snprintf(message_with_log_level, CONFIG_LOG_MESSAGE_CHAR_BUFFER_LEN, "%s: %s",
           macki_log_level_to_string(level), message);
  va_list args;
  va_start(args, message);
  char new_message[CONFIG_LOG_MESSAGE_CHAR_BUFFER_LEN];
  vsnprintf(new_message, CONFIG_LOG_MESSAGE_CHAR_BUFFER_LEN,
            message_with_log_level, args);
  va_end(args);
