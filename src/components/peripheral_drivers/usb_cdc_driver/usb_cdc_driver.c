...
void usb_cdc_init(usb_cdc_driver_t *usb_cdc_driver) {
  tinyusb_driver_install(&usb_cdc_driver->tusb_cfg);
  tusb_cdc_acm_init(&usb_cdc_driver->acm_cfg);
...
bool usb_cdc_send(usb_cdc_driver_t *usb_cdc_driver, uint8_t *data,
  ...
  ret |= tinyusb_cdcacm_write_queue(usb_cdc_driver->cdc_port, data, length);
  ret |= tinyusb_cdcacm_write_flush(usb_cdc_driver->cdc_port, USB_CDC_TRANSFER_TIMEOUT);
  return ret == ESP_OK;
}
