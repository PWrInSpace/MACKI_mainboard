...
static void ring_buffer_reset() {
  ...
  // }
  ring_buffer_init(&test_ring_buffer, CONFIG_MAX_RING_BUFFER_SIZE, sizeof(uint16_t));
}

TEST_CASE("ring_buffer_init Test Fail", "[RING_BUFFER]") {
  ring_buffer_status_t ret =
      ring_buffer_init(&test_ring_buffer, CONFIG_MAX_RING_BUFFER_SIZE + 10, sizeof(uint16_t));
